...
for (team in team.name.list){
    ...
    for (year in year.list[-1]){
        ...
        if (length(college.list)<1) {
            # if the old college list had more than one entry
            if(length(old.college.list)>1){
                # loop through each college in the list
            ...
            else { # this case is when there is only one element. Could get rid of this?
                #create a vector to add the name team and year to running list
                add.to.list<-c(team,old.college.list,last.year)
                #bind the row just created to the running list
                running.list<-rbind(running.list,add.to.list)
                #break loop after writing the entry to list
                break
            }
        }
